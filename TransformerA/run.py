...
Options:
    ...
    --model-path=<file>                     model file path
    --embed-size=<int>                      embedding size [default: 512]
    --nhead=<int>                           number of heads [default: 8]
...
# Tell PyTorch it's safe to unpickle your custom Vocab class
...
def compute_corpus_level_bleu_score(references: List[List[str]], hypotheses: List[Hypothesis]) -> float:
    ...
    detokened_refs = [''.join(pieces).replace('▁', ' ').strip() for pieces in references]
    detokened_hyps = [''.join(hyp.value).replace('▁', ' ').strip() for hyp in hypotheses]
...
def train(args: Dict):
    ...
    """
    train_data_src = read_corpus(args['--train-src'], source='src', vocab_size=20000)
    train_data_tgt = read_corpus(args['--train-tgt'], source='tgt', vocab_size=20000)

    dev_data_src = read_corpus(args['--dev-src'], source='src', vocab_size=10000)
    dev_data_tgt = read_corpus(args['--dev-tgt'], source='tgt', vocab_size=10000)
...
def decode(args: Dict[str, str]):
    ...
    test_data_src = read_corpus(args['TEST_SOURCE_FILE'], source='src', vocab_size=10000)
    if args['TEST_TARGET_FILE']:
        print("load test target sentences from [{}]".format(args['TEST_TARGET_FILE']), file=sys.stderr)
        test_data_tgt = read_corpus(args['TEST_TARGET_FILE'], source='tgt', vocab_size=10000)
