...
def compute_corpus_level_bleu_score(references: List[List[str]], hypotheses: List[Hypothesis]) -> float:
    ...
    detokened_refs = [''.join(pieces).replace('▁', ' ').replace(' ', ' ').strip() for pieces in references]
    detokened_hyps = [''.join(hyp.value).replace('▁', ' ').replace(' ', ' ').strip() for hyp in hypotheses]
...
def train(args: Dict):
    ...
    """
    train_data_src = read_corpus(args['--train-src'], source='src', vocab_size=20000)       # EDIT: NEW VOCAB SIZE
    ...
    dev_data_tgt = read_corpus(args['--dev-tgt'], source='tgt', vocab_size=3000)
