...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python run.py train --train-src=./ja_en_data/kftt-data-1.0/data/orig/kyoto-train.ja --train-tgt=./ja_en_data/kftt-data-1.0/data/orig/kyoto-train.en --dev-src=./ja_en_data/kftt-data-1.0/data/orig/kyoto-dev.ja --dev-tgt=./ja_en_data/kftt-data-1.0/data/orig/kyoto-dev.en --vocab=vocab.json --cuda --lr=1e-3 --patience=1 --valid-niter=1000 --batch-size=32 --dropout=.3
elif [ "$1" = "test" ]; then
